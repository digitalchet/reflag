...
for (const code of extraRgiCodes) {
  if (!fs.existsSync(path.join(root, 'assets', 'flags', `${code}.svg`))) throw new Error(`Missing RGI flag asset: ${code}`);
  if (!new RegExp(`\\b${code.toUpperCase()}\\b`).test(content)) throw new Error(`Missing RGI flag registry entry: ${code}`);
}

console.log(JSON.stringify({ globalHttpHttps: true, noBackgroundWorker: true, permissions: manifest.permissions, selectedWebsiteGuard: true, legacyMigration: true, extraRgiFlags: extraRgiCodes.length }));
